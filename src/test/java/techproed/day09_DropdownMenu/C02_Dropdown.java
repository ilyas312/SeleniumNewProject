...
public class C02_Dropdown {
    ...
    Dropdown'u handle(outomate) etmek için;
        ...
        3-Select objesinin dropdown menü Webelement inin içeriğine ve seçeneklerine
          erişim sağlaması için SELECT sınıfına argüman olarak locate ettiğimiz
          webelementi koyarız
        SYNTAX :
            Select select=new Select(ddm webelementi)
        4-Select class ı sadece <select> tag ı ile oluşturulmuş dropdown menülere
          uygulanabilir
        5-select objesi dropdown menüyü handle edebilmek için 3 method kullanır
          -selectByvisibleText() -> Dropdown menü deki elemente görünür metin ile
          ulaşmak için kullanılır
          -selectByIndex() -> Index ile ulaşmak için kullanılır(Index 0(sefır)dan başlar)
          -selectByValue() -> Elementin değeri ile ulaşmak için kullanılır(option tag'larındaki değer(value) ile)

        6-getOptions() -> Locate ettiğimiz ddm deki tüm seçenekleri bize verir.(List e atıp loop ile yazdıra biliriz)
        7-getFirstSelectedOption() -> Ddm'deki seçili kalan seçeneği bize verir.
          Birden fazla seçenek seçildiyse, bu seçilenlerin ilkini verir.
        8-Ddm ye sendKeys() method u ile de dropdown menü deki seçeneklerden birini
          kullanarak gönderebiliriz
     */
...
*/
    ...
    public void test01() {
        ...
        //c. Select object i kullaarak 3 farkli sekilde secim yapiniz
        select.selectByIndex(5);// 2018 i seçer
    ...
    public void test02() {
        ...
        //getOptions() method u ile tüm seçenekleri yazdırdık

        //2. yol
        /*
        System.out.println("*********************************");
        List<WebElement> eyaletlerr=driver.findElements(By.xpath("//*[@id='state']"));
